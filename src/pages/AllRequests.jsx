  ...
  const changeStatus = async (req, newStatus) => {
    try {
      ...
      if (newStatus === "completed") {
        await addDoc(collection(db, "archivedRequests"), { ...req, status: "completed" });
    ...
    <div className={styles.container}>
      ...
      {statusOrder.map(status => (
        groupedRequests[status].length > 0 && (
          <div key={status}>
            <h2>{status.toUpperCase()} ({groupedRequests[status].length})</h2>
              ...
              {groupedRequests[status].map(req => (
                <div key={req.id} className={styles.card}>
                  ...
                  <div className={styles.buttons}>
                    {req.status === "pending" && (
                      <>
                        <button onClick={() => changeStatus(req, "accepted")}>Accept</button>
                        <button onClick={() => changeStatus(req, "rejected")}>Reject</button>
                      </>
                    )}
                    {req.status === "accepted" && <button onClick={() => changeStatus(req, "in-progress")}>Start</button>}
                    {req.status === "in-progress" && <button onClick={() => changeStatus(req, "completed")}>Complete</button>}
                  </div>
                </div>
              ))}
            </div>
